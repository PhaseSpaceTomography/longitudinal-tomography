...
def save_profile_ftn(profiles: np.ndarray, recprof: int, output_dir: str):
...
def save_self_volt_profile_ftn(self_fields: np.ndarray, output_dir: str):
...
def save_phase_space_ftn(image: np.ndarray, recprof: int, output_path: str):
...
def write_plotinfo_ftn(machine: 'Machine', particles: 'Particles',
...
def save_difference_ftn(diff: np.ndarray, output_path: str, recprof: int):
...
def print_tracking_status_ftn(ref_prof: int, to_profile: int):
