...
py::tuple wrapper_kick_and_drift_int_scalar(
        ...
        const int_Tarr &input_denergy,
        const int_Tarr &input_dphi,
        const int_Tarr &input_rf1v,
        const int_Tarr &input_rf2v,
        const int_Tarr &input_phi0,
        const int_Tarr &input_deltaE0,
    ...
    py::capsule capsule(ptr_phi12, [](void *p) { delete[] reinterpret_cast<int_t *>(p); });
    int_Tarr arr_phi12({nturns}, ptr_phi12, capsule);

    wrapper_kick_and_drift_int_array(input_xp, input_yp, input_denergy, input_dphi, input_rf1v, input_rf2v, input_phi0,
                                 input_deltaE0,
                                 input_drift_coef, arr_phi12, hratio, dturns, rec_prof, deltaturn, nturns, nparts, ftn_out, S, G, 
                                 abs_of_lowest_possible_angle, callback);
...
py::tuple wrapper_kick_and_drift_int_array(
    ...
    auto *xp = static_cast<int_t *>(xp_buffer.ptr);
    auto *yp = static_cast<int_t *>(yp_buffer.ptr);

    const int n_profiles = xp_buffer.shape[0];
    auto **const xp_d = new int_t *[n_profiles];
    for (int i = 0; i < n_profiles; i++)
        xp_d[i] = &xp[i * nparts];

    auto **const yp_d = new int_t *[n_profiles];
    for (int i = 0; i < n_profiles; i++)
        yp_d[i] = &yp[i * nparts];
    ...
    int_t *const denergy = static_cast<int_t *>(denergy_buffer.ptr);
    int_t *const dphi = static_cast<int_t *>(dphi_buffer.ptr);
    const int_t *const rf1v = static_cast<int_t *>(rf1v_buffer.ptr);
    const int_t *const rf2v = static_cast<int_t *>(rf2v_buffer.ptr);
    const int_t *const phi0 = static_cast<int_t *>(phi0_buffer.ptr);
    const int_t *const deltaE0 = static_cast<int_t *>(deltaE0_buffer.ptr);
    const int_t *const phi12 = static_cast<int_t *>(phi12_buffer.ptr);
    const int_t *const drift_coef = static_cast<int_t *>(drift_coef_buffer.ptr);
    ...
    try {
        kick_and_drift_int<int_t>(xp_d, yp_d, denergy, dphi, rf1v, rf2v, phi0, deltaE0, drift_coef,
                       phi12, hratio, dturns, rec_prof, deltaturn, nturns, nparts, ftn_out, S, G, abs_of_lowest_possible_angle, cb);
    } catch (const std::exception &e) {
        cleanup();
        throw;
    }
...
"xp"_a, "yp"_a, "denergy"_a, "dphi"_a, "rfv1"_a, "rfv2"_a, "phi0"_a,
"deltaE0"_a, "drift_coef"_a, "phi12"_a, "h_ratio"_a, "dturns"_a,
"rec_prof"_a, "deltaturn"_a, "nturns"_a, "nparts"_a, "ftn_out"_a = false, "S"_a = 1, "G"_a = 1, "abs_of_lowest_possible_angle"_a = 0, "callback"_a = py::none()
);
