...
class SysHandling:
    ...
    def __init__(self, input_list, working_dir, resources_dir, py_main_pth):
        self.input_list = input_list
        self.working_dir = working_dir
        self.resources_dir = resources_dir
        self.py_main_pth = py_main_pth
    ...
    def run_programs(self, index, fortran=True, python=True):
        ...
        command = ("cp " + self.resources_dir + self.input_list[index] + "/"
                   + self.input_list[index] + ".dat "
                   + self.working_dir[:-5] + "/input_v2.dat")
        os.system(command)
        # Running python script
        if python:
            print("Running python")
            # TODO: Try to catch exeptions
            subprocess.call(['python', self.py_main_pth])

        # Running fortran script
        if fortran:
            print("running fortran")
            input_file = open("./input_v2.dat")
            proc = subprocess.call(["./tomo_vo.intelmp"], stdin=input_file)
    ...
    def find_images(dirr):
        ...
        if len(python_image) > 1 or len(fortran_image) > 1:
            logging.warning("Moore than two images were found.")
        elif len(python_image) == 0 or len(fortran_image) == 0:
            logging.warning("One of the images were not found")
